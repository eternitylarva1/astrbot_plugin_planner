...
class ApiClient:
    ...
    async def update_event(self, event_id: int, event_data: Dict) -> Optional[Dict]:
        ...
        Returns:
            Updated event dict
        """
        return await self._request("PUT", f"/api/events/{event_id}", json_data=event_data)
    ...
    async def llm_create(self, text: str) -> Optional[List[Dict]]:
        ...
        """
        result = await self._request("POST", "/api/llm/create", json_data={"text": text})
        return result if isinstance(result, list) else None
    ...
    async def llm_command(self, text: str, dry_run: bool = False) -> Optional[Dict]:
        ...
        return await self._request(
            "POST", 
            "/api/llm/command", 
            json_data={"text": text, "dry_run": dry_run}
        )
